    ...
    <div className="px-5 pt-6 pb-4">
      ...
      <Section title="2. 取得する情報">
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li>年齢</li>
          <li>性別（男性／女性／その他／回答しない から選択）</li>
          <li>
            配布元の識別子（配布物のQRコードに含まれる符号。所属団体をまとめて集計するために使用し、個人を識別するものではありません）
          </li>
        </ul>
        <p className="mt-2">
          氏名・生年月日・住所・電話番号は取得しません。上記以外の情報を取得する必要が生じた場合は、本ポリシーを改定し、改めて同意をいただいたうえで取得します。
        </p>
      </Section>

      <Section title="3. 利用目的">
        <ul className="list-disc space-y-1 pl-5">
          <li>ご本人が記録した観察データを保存し、別の端末でも表示・継続できるようにするため</li>
          <li>ログイン（本人確認）のため</li>
          <li>
            年齢・性別・配布元の識別子は、個人が特定されない形にまとめた統計として、配布元の団体へ全体の傾向を報告するため
          </li>
        </ul>
        <p className="mt-2">これら以外の目的では利用しません。</p>
      </Section>

      <Section title="4. 第三者提供・委託">
        <p>
          個人を識別できる形で第三者へ提供することはありません。配布元の団体へは、個人を特定できない形にまとめた集計値（人数・割合・分布など）のみを報告します。集計の対象人数が少ない場合は、個人が推測されないよう数値を表示しません。
        </p>
        <p className="mt-2">
          データの保管にはクラウドサービス（Supabase）を利用しています（保管リージョン：東京）。
        </p>
      </Section>
      ...
      <Section title="6. データの削除">
        <p>
          利用者は、本サービス内の「記録データを削除する」機能から、いつでもご自身の観察記録および登録情報（ログイン用メールアドレス・年齢・性別を含む）を完全に削除できます。削除したデータは元に戻せません。
        </p>
      </Section>
      ...
      <Section title="8. 改定">
        <p>
          本ポリシーの内容は、必要に応じて改定することがあります。取得する情報や利用目的を変更する場合は、改定のうえ、改めて同意をいただきます。
        </p>
      </Section>
      ...
      <p className="mt-6 text-xs text-zinc-400">
        制定日：2026年6月5日
        <br />
        最終改定日：2026年8月7日（版 {POLICY_VERSION}）
      </p>
